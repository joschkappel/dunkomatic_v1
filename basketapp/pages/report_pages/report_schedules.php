...
while (!$rs2->EOF){
	...
	$schedule[$rs2->fields['gamedate'].strval($rs2->fields['weekend'])][$rs2->fields['group_name']] = $rs2->fields['game_day'];
 	$schedule_dow[$rs2->fields['gamedate'].strval($rs2->fields['weekend'])] = $rs2->fields['dow'];

	$rs2->MoveNext();
}

// ksort($schedule);
// array_walk($schedule, 'ksort');
// ksort($schedule_dow);
// array_walk($schedule_dow, 'ksort');
...
foreach ($dates as $date){
	...
	if ( $schedule_dow[$date] == 7  ){
		if (substr($date, -1) == "1"){
			$txtdate = substr_replace($date ,"", -1)." (SA/SO)";
		} else {
			$txtdate = substr_replace($date ,"", -1)." (SA)";
		}
	} elseif ( $schedule_dow[$date] == 1 ){
		$txtdate = substr_replace($date ,"", -1)." (SO)";
	} else {
		$txtdate = substr_replace($date ,"", -1)." (??)";
	}
