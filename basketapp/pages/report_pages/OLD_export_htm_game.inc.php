...
$wdays = array("So","Mo","Di","Mi","Do","Fr","Sa");
		...
		fputs($file,"<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\"".
				...
				"<head>".
				"<title>HBV ".$_SESSION['CONFIG_region']." - Bezirksspielplan - ".$_SESSION['CONFIG_season']."</title>".
		...
		$sql2="SELECT DISTINCT l.shortname, g.game_no, g.game_team_home, g.game_team_guest, DATE_FORMAT(g.game_date,'%w') as gamewday, DATE_FORMAT(g.game_date,'%d.%m.%Y') as game_datef, TIME_FORMAT(g.game_time,'%H:%i' ) as game_timef, g.game_gym, g.game_team_ref1,g.game_team_ref2 FROM game g, league l, club c WHERE g.league_id=l.league_id AND ".$sCLUBregion." AND (g.club_id=c.club_id OR g.club_id_guest=c.club_id) ORDER BY g.game_date, g.game_time, g.game_team_home";			
