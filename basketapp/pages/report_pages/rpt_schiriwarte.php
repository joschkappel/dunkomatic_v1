...
$filename = $exp_dir.'/Adressen_Schiedsrichterwarte';	
$rptTitle = 'Adressen aller Schiedsrichterwarte';
$desc = 'Adressen aller Schiedsrichterarte des Bezirks';

$wbook = $report->createWorkbook( $filename, $rptTitle, '', $desc );
	
// all leaders all clubs
$report->addSheet( 0, $rptTitle, 'Schiedsrichterwarte '.$sSQLregion, $sSQLregion, $lRegion, $season, false );
$sqlGroup = 'SELECT CONCAT_WS(\' \', c.shortname, c.name), CONCAT( m.firstname,\' \', m.lastname), m.street, CONCAT_WS(\' \',m.zip, m.city), m.email, m.phone1, m.phone2, m.mobile, m.fax1, m.email2 FROM member as m, club as c WHERE m.member_role_id = 1 AND m.club_id=c.club_id AND c.region=\''.$sSQLregion.'\' ORDER BY c.shortname';
