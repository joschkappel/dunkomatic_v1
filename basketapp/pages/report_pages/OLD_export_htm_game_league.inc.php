...
while (!$rs->EOF) {
		...
		$exp_file = $exp_dir.'/R_'. $fshort .'_Rundenspielplan.html';
		...
		fputs($file,"<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\"".
				...
				"<head>".
				"<title>HBV ".$_SESSION['CONFIG_region']." - Rundenspielplan ".$rs->fields["shortname"]." - ".$_SESSION['CONFIG_season']."</title>".
		...
		// get league lead
		$sql2='SELECT lastname, firstname, mobile, city, zip, street, phone1, fax1, email FROM member WHERE league_id=\''.$rs->fields["league_id"].'\' AND member_role_id=2 ';			
		...
		// get trikotfarben
		$sql2='SELECT g.game_team_guest, t.color, t.firstname, t.lastname, t.phone1, t.phone2, t.email FROM game g, team t WHERE g.league_id='.$rs->fields["league_id"].' AND g.team_id_guest=t.team_id GROUP BY g.game_team_guest ORDER BY g.game_team_guest';			
		$rs2 = $conn->Execute($sql2);
		$i=0;
		...
		while (!$rs2->EOF) {
			...
			fputs($file,"<td>".$rs2->fields["firstname"]." ".$rs2->fields["lastname"]."</td>");
			fputs($file,"<td>".$rs2->fields["phone1"]."<br/>".$rs2->fields["phone2"]."</td>");
			fputs($file,"<td>".$rs2->fields["email"]."</td>");
			fputs($file,"</tr>");
