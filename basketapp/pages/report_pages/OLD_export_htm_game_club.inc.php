...
//		$exp_dir = DDIR_CLUBS.'/'.$rs->fields["shortname"];
		$exp_dir = DDIR_CLUBS;
		$exp_file = $exp_dir.'/V_'. $rs->fields["shortname"] .'_Vereinsspielplan.html';
		...
		fputs($file,"<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\"".
				...
				"<head>".
				"<title>HBV ".$_SESSION['CONFIG_region']." - Vereinsspielplan ".$rs->fields["shortname"]." - ".$_SESSION['CONFIG_season']."</title>".
				"<style type=\"text/css\">@page { size:landscape; }</style>".
				"</head><body>".
				"<table cellpadding=\"2\" cellspacing=\"1\" width=\"95%\">".
				"<tr><th align=\"left\" width=\"60%\">".
				"HBV ".$_SESSION['CONFIG_region']." </th>".
				"<th align=\"right\">Saison ".$_SESSION['CONFIG_season']."</th>".
				"</tr><tr></tr><tr>".
				"<th align=\"left\" width=\"60%\">Vereinsspielplan für ".$rs->fields["name"].
		...
		$sql2='SELECT l.shortname, g.game_no, g.game_team_home, g.game_team_guest, DATE_FORMAT(g.game_date,\'%w\') as gamewday, DATE_FORMAT(g.game_date,\'%d.%m.%Y\') as game_datef, TIME_FORMAT(g.game_time,\'%H:%i\' ) as game_timef, g.game_gym, g.game_team_ref1,g.game_team_ref2 FROM game g, league l WHERE (club_id='.$rs->fields["club_id"].' OR club_id_guest='.$rs->fields["club_id"].') AND g.league_id=l.league_id ORDER BY g.game_date, g.game_time, g.game_gym';			
		...
		fputs($file,"<h2>Hallenadressen für Auswärtsspiele</h2>".
		...
		// get arenas
		$sql2='SELECT c.shortname, gy.shortname as gym, gy.name, gy.zip, gy.city, gy.street FROM game g, club c, gymnasium gy WHERE g.club_id_guest='.$rs->fields["club_id"].' AND g.club_id=c.club_id AND gy.club_id=g.club_id GROUP BY c.shortname, gy.shortname ORDER BY c.shortname, gy.shortname';			
		$rs2 = $conn->Execute($sql2);
