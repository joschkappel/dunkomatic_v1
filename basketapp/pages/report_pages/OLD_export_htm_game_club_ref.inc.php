...
//		$exp_dir = DDIR_CLUBS.'/'.$rs->fields["shortname"];
		...
		fputs($file,"<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\"".
				...
				"<head>".
				"<title>HBV ".$_SESSION['CONFIG_region']." - Schirieinsatzplan ".$rs->fields["shortname"]." - ".$_SESSION['CONFIG_season']."</title>".
		...
		// get arenas
		$sql2='SELECT c.shortname, gy.shortname as gym, gy.name, gy.zip, gy.city, gy.street FROM game g, club c, gymnasium gy WHERE (game_team_ref1=\''.$rs->fields["shortname"].'\' OR game_team_ref2=\''.$rs->fields["shortname"].'\') AND g.club_id=c.club_id AND gy.club_id=g.club_id GROUP BY c.shortname, gy.shortname ORDER BY c.shortname, gy.shortname';			
		$rs2 = $conn->Execute($sql2);
