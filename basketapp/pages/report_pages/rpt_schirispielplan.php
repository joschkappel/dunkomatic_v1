...
while (!$rs->EOF) {
		...
		$filename = $exp_dir.'/V_'. $fshort .'_Schiriplan';	
		$rptTitle = 'Schirieinsatzplan '.$rs->fields["name"];
		$desc = 'Alle Schiedsrichtereinsaetze eines Vereins der aktuellen Saison';
		...
		$sqlGroup = 'SELECT CONCAT( CASE (DATE_FORMAT(g.game_date,\'%w\')) WHEN 0 THEN \'So, \' WHEN 1 THEN \'Mo, \' WHEN 2 THEN \'Di, \' WHEN 3 THEN \'Mi, \' WHEN 4 THEN \'Do, \' WHEN 5 THEN \'Fr, \' WHEN 6 THEN \'Sa, \' ELSE \'??, \' END, DATE_FORMAT(g.game_date,\'%d.%m.%Y\')), TIME_FORMAT(g.game_time,\'%H:%i\' ),l.shortname, g.game_no, g.game_team_home, g.game_team_guest,  g.game_gym, g.game_team_ref1,g.game_team_ref2 FROM game g, league l WHERE (game_team_ref1=\''.$rs->fields["shortname"].'\' OR game_team_ref2=\''.$rs->fields["shortname"].'\' OR (game_team_ref1=\'****\' AND SUBSTRING(game_team_home,1,4)=\''.$rs->fields["shortname"].'\' )) AND g.league_id=l.league_id  ORDER BY g.game_date, g.game_time, l.shortname, g.game_no';		
		...
		$sqlGroup = 'SELECT c.shortname, gy.shortname as gym, gy.name, gy.zip, gy.city, gy.street FROM game g, club c, gymnasium gy WHERE(game_team_ref1=\''.$rs->fields["shortname"].'\' OR game_team_ref2=\''.$rs->fields["shortname"].'\') AND g.club_id=c.club_id AND gy.club_id=g.club_id GROUP BY c.shortname, gy.shortname ORDER BY c.shortname, gy.shortname';
		$report->createSheetContent( Reporter::SHEET_LIST, $sqlGroup, '', '', $colHdr  );
