...
if( !defined( "DB_SIMPLE_XLS_GEN" ) ) {
   ...
   Class Db_SXlsGen extends SimpleRepGen {
      ...
      function FetchData()
      {
         switch ( $this->db_type ) {
            case "mysql":
                  ...
                  mysql_db_query( $this->db_name, "set character set utf8", $this->db_con_id );
      ...
      function GetXlsFromQuery( $query )
      {
       ...
       if ( count($this->headerline) > 0 ) {
		...
		$this->WriteSpreadsheet($this->fileFormat); 
        	echo 'Report '.$this->filename." saved\n";
        	return true;
