...
<!-- ------Object table list  -------  -->
<form method="get" name="<? echo $obj_name ?>_relate_form" action="" target="">
...
while (!$rs->EOF){
	...
	?>
    <tr>
    <td bgcolor="<? echo $bgcolor?>"><input type="checkbox" <? echo $checked ?> name="secondary_selected_ids_arr[]"  value="<? echo $rs->fields[$secondary_object["id_column_name"]];  ?>"></td>
    <?
    foreach  ($fields_arr as $field){
        if ($field->show_in_list)
        {
             ...
             {
                 ?><td <?php echo $click_mouse; ?> bgcolor="<?php echo $bgcolor; ?>" dir="<? echo $field->lang_dir ?>"><? if (format_from_db_date($rs->fields[$field->name],$field->date_format)) echo format_from_db_date($rs->fields[$field->name],$field->date_format); else echo "&nbsp;" ?></td><?
             }
    ...
    foreach ($relation_fields_arr as $field)
	{
        if ($field->show_in_list)
        {
            if ($checked=="checked")
            {
					 ...
					 {
						 ?><td class="<? echo $css?>" dir="<? echo $field->lang_dir ?>"><? if ($relation_records_by_secondary_id_arr[$rs->fields[$secondary_object["id_column_name"]]][$field->name]) echo html_entity_decode($relation_records_by_secondary_id_arr[$rs->fields[$secondary_object["id_column_name"]]][$field->name]); else echo "&nbsp;";?></td><?
					 }
					 if ($field->type=="checkbox")
					 {
						 ?><td class="<? echo $css?>"><? if ($relation_records_by_secondary_id_arr[$rs->fields[$secondary_object["id_column_name"]]][$field->name]) echo sNO; else echo sYES;?></td><?
					 }
					 if ($field->type=="datetime" || $field->type=="datetime_now")
					 {
						 ?><td class="<? echo $css?>" ><? if (format_from_db_date($relation_records_by_secondary_id_arr[$rs->fields[$secondary_object["id_column_name"]]][$field->name],$field->date_format)) echo format_from_db_date($relation_records_by_secondary_id_arr[$rs->fields[$secondary_object["id_column_name"]]][$field->name],$field->date_format); else echo "&nbsp;" ?></td><?
					 }
					 if ($field->type=="selectboxdb")
					 {
						if ($rs->fields[$field->name])
						{
							...
							{
								?><td class="<? echo $css?>" ><? echo $rs_select_box->fields[$field->display_field_name]; ?></td><?
							}
							else
							{
								?><td class="<? echo $css?>">&nbsp;</td><?
							}
						}
						else
						{
					 ...
					 if ($field->type=="selectboxlist")
					 {
						 ...
						 ?><td class="<? echo $css?>" ><?  if ($list_value) echo $list_value; else echo "&nbsp;" ?></td><?
					 }
					 if ($field->type=="active")
					 {
						 ?><td class="<? echo $css?>"><? if ($relation_records_by_secondary_id_arr[$rs->fields[$secondary_object["id_column_name"]]][$field->name]) echo YES; else echo NO;?></td><?
					 }
			}
			else
			{
...
if (isset($actions_arr))
{
	...
	foreach ($actions_arr as $action){
		...
		{?>  markierte:  <a href="javascript:<? echo $action["onclick"] ?>"><? echo $action["heading"] ?></a><?php
		}
	}
...
<input type="hidden" name="obj_name" value="<? echo $obj_name ?>">
<input type="hidden" name="secondary_all_ids" value="<? echo implode(",",$secondary_all_ids_arr); ?>">
<input type="hidden" name="secondary_pre_selected_ids" value="<? echo implode(",",$secondary_ids_arr); ?>">
...
<input type="hidden" name="secondary_id_column_name" value="<? echo $secondary_object["id_column_name"] ?>">
</form>


<form method="get" name="<? echo $obj_name ?>_delete_all_form" action="" target="">
...
{
	document.<? echo $obj_name ?>_relate_form.submit();
}
function delete_all_relations(message)
{
	...
	document.<? echo $obj_name ?>_delete_all_form.submit();
}
</script>
