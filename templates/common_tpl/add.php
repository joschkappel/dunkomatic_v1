...
<!-- --------------------------------- BODY --------------------------------- -->
<!-- calendar scripts -->
<link rel="stylesheet" type="text/css" media="all" href="<? echo $APLICATION_ROOT ?>common/jscalendar-1.0/calendar-brown.css" title="calendar-system" />
<script type="text/javascript" src="<? echo $APLICATION_ROOT ?>common/jscalendar-1.0/calendar.js"></script>
<script type="text/javascript" src="<? echo $APLICATION_ROOT ?>common/jscalendar-1.0/lang/calendar-en.js"></script>
<script type="text/javascript" src="<? echo $APLICATION_ROOT ?>common/jscalendar-1.0/calendar-setup.js"></script>
<!-- calendar scripts -->
...
function add_object(  )
{
    ...
    foreach ($fields_arr as $field){
             if ($field->show_in_add)
			 {
				 ...
				 {
                     ?>
                    if (document.<? echo $obj_name ?>_add_form.<? echo $field->name ?>.value=="")
                    {
                        alert('<? echo '"'.$field->get_field_heading().'" '. $sIsMandatory ?>');
                        return;
                    }
                     <?
				 }
...
foreach ($fields_arr as $field){
    ...
    if (($field->show_heading)AND (!$field->isAutoCreate))
    {
        ...
        ?>
        <tr>
        <td class="OTRecordHeadCell"><? echo $field->get_field_heading();   if ($field->isMandatory) echo '*'; ?></td>
        <?
    }
    if (($field->type=="text" || $field->type=="url" ||$field->type=="url_text" ) AND (!$field->isAutoCreate))
	{
	
        ?><td class="<?php echo ($GLOBALS['validation_results'][$field->name]!="") ? 'tblWarn' : 'OTRecordDataCell'; ?>"><input type="text" name="<? echo $field->name ?>" class="<? echo $field->css_class ?>" value="<?php echo $fvalue; ?>" dir="<? echo $field->lang_dir ?>"></td><?
    }
    if (($field->type=="checkbox") AND (!$field->isAutoCreate))
	{
        ...
        ?><td class="OTRecordDataCell"><input type="checkbox" <?php echo $checked ?> name="<? echo $field->name ?>" class="<? echo $field->css_class ?>" value="1"></td><?
    }
    ...
    {
        ?><input type="hidden" name="<? echo $field->name ?>_number_of_digits" value="<? echo $field->number_of_digits ?>"><?
    }
	...
	{
        ?><td class="OTRecordDataCell"><textarea name="<? echo $field->name ?>" cols="<? echo $field->cols ?>" rows="<? echo $field->rows ?>" class="<? echo $field->css_class ?>" dir="<? echo $field->lang_dir ?>"></textarea></td><?
    }
    if (($field->type=="selectboxdb") AND (!$field->isAutoCreate))
	{
 	   ...
 	   if ( isset($_SESSION["select_cache"][$field->table_name.'_'.$field->name.'_'.$field->display_field_name.'_'.$field->where_clause])) {
			...
			while ($dispval = current($_SESSION["select_cache"][$field->table_name.'_'.$field->name.'_'.$field->display_field_name.'_'.$field->where_clause])){
				?><option value="<? echo key($_SESSION["select_cache"][$field->table_name.'_'.$field->name.'_'.$field->display_field_name.'_'.$field->where_clause]) ?>"><? echo $dispval ?></option><?
			...
			{
				?><option value="<? echo $rs_select_box->fields[$field->save_field_name] ?>"><? echo $rs_select_box->fields[$field->display_field_name] ?></option><?
    ...
    if (($field->type=="password") AND (!$field->isAutoCreate))
	{
		...
		{
			?>
			&nbsp;&nbsp;&nbsp;<span class="RePasswordHeading"><? echo $field->get_repassword_heading(); ?></span>&nbsp;<input type="password" name="re_<? echo $field->name ?>" class="FormObjects" value="" dir="<? echo $field->lang_dir ?>">
			<?
		}
		?>
		</td><?
    ...
    if (($field->type=="datetime") AND (!$field->isAutoCreate))
	{
		...
		<table cellspacing="0" cellpadding="0" style="border-collapse: collapse"><tr>
		 <td><input type="text" name="<? echo $field->name ?>" class="<? echo $field->css_class ?>" dir="ltr" id="<? echo $field->name ?>" value="<?php echo $fvalue; ?>" readonly="1" /></td>
		 <td><img src="<? echo $APLICATION_ROOT ?>common/jscalendar-1.0/img.gif" id="<? echo $field->name ?>_cal" style="cursor: pointer; border: 1px solid red;" title="Date selector"
			  onmouseover="this.style.background='red';" onmouseout="this.style.background=''" /></td>
		</table>		
		<script type="text/javascript">
			Calendar.setup({
				inputField     :    "<? echo $field->name ?>",      // id of the input field
				ifFormat       :    "<? echo DB_DATE_FORMAT ?>",       // format of the input field
    ...
    {
     	?>
		<td class="OTRecordRemarkCell"><? echo $field->get_field_remark() ?></td>
		<?php
    }
