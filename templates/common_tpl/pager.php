...
foreach  ($fields_arr as $field_list){
   if ($field_list->type == 'selectboxdb'){
    ...
    if ( (! isset($_SESSION["select_cache"][$field_list->table_name.'_'.$field_list->name.'_'.$field_list->display_field_name.'_'.$field_list->where_clause])) || ( count($_SESSION["select_cache"][$field_list->table_name.'_'.$field_list->name.'_'.$field_list->display_field_name.'_'.$field_list->where_clause])==0) ){


   	   		if ($field_list->where_clause<>""){
     			$sql2="SELECT `".$field_list->display_field_name."`, `".$field_list->save_field_name."` FROM ".$field_list->table_name." WHERE ".$field_list->where_clause ;
   	   		} else {
		...
		$_SESSION["select_cache"][$field_list->table_name.'_'.$field_list->name.'_'.$field_list->display_field_name.'_'.$field_list->where_clause]=$tmpcache;
...
if ((isset($_SESSION[$obj_name.'_sort_by_session'])) AND ( !$force_sort))
{
    $sort_by=$_SESSION[$obj_name.'_sort_by_session'];
}
if ((isset($_SESSION[$obj_name.'_sort_type_session'])) AND (!$force_sort))
{
...
while (!$rs->EOF){
	...
	if ((USE_SECURITY) AND ($sec_level==1))
	{
		if ((!isset($allowed_ids)) OR ( isset($allowed_ids) AND ((!in_array( $rs->fields[$id_column_name], $allowed_ids ) AND ($allowed_ids[0]!='*')))))
			{$can_modify = FALSE;
			} else
		if ((USE_SECURITY) AND ($sec_level>2))
		{ $can_modify = FALSE;}
	}

	if (( $_SESSION["region"] !='') AND ( $rs->fields["region"] != '')) {
		if ( $_SESSION["region"] != $rs->fields["region"] ) {
			$can_modify = FALSE;
		}
	}

	if ($can_modify) { $can_add = TRUE;}
		...
		foreach ($in_table_lactions_arr as $in_table_action)
		{
			...
			if (USE_SECURITY){
				if ($in_table_action["type"] == 'R'){
					...
					if (($sec_level==2) OR ($sec_level==4)){
						$show_action= $can_modify;
					}
			}
			}

			if ($show_action AND (isset($rs->fields["region"])) AND ($_SESSION["region"] != $rs->fields["region"])){
				$show_action = FALSE;
			}
	...
	if (isset($in_table_actions_arr))
	{
		foreach ($in_table_actions_arr as $in_table_action)
		{
			...
			if ($show_action AND (isset($rs->fields["region"])) AND ($_SESSION["region"] != $rs->fields["region"]) AND (($in_table_action["action"]=="games()") OR ($in_table_action["action"]=="cleanup()"))){
				$show_action = FALSE;
			}
...
if (isset($actions_arr))
{
		...
		if ((USE_SECURITY)AND (($sec_level==1) OR ($sec_level==0)) AND $can_add ){
...
if (isset($actions_arr))
{
	...
	foreach ($actions_arr as $action){
	   if ( ((USE_SECURITY)AND (($sec_level==1) OR ($sec_level==0)) AND $can_add AND ($action["action"]=="add")) OR ($action["action"]=="navback"))
		{
...
function <?php echo $obj_name ?>_perform_action(action)
{
    ...
    if (action=="mail")
    {
        document.<?php echo $obj_name ?>_actions_form.action="<?php echo $obj_name ?>_mail.php";
    }
    if (action=="duplicate")
    {
...
{
     document.<?php echo $obj_name ?>_actions_form.<?php echo $obj_name ?>_id_selected.value=obj_id;
     document.<?php echo $obj_name ?>_actions_form.parent_shortname.value=obj_shortname;
