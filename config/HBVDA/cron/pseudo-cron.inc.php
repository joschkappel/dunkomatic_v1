...
function getLastActualRunTime($jobname) {
	...
	if (file_exists($jobfile)) {
		...
		if ($debug) print ("job file ".$jobfile." does not exist\n");
	}
	return 0;
}
...
function markLastRun($jobname, $lastRun) {
	$jobfile = getJobFileName($jobname);
	if (touch($jobfile)){
	   if ($debug) print "$FileName modification time has been changed to todays date and time\n"; 
		} else { 
   	   if ($debug) print "Sorry Could Not change modification time of $FileName\n"; 
	}
